...
const Person = ({...actor}) => {
  ...
  return (
    <Link to={`person/${id}`} className="cursor-pointer z-[999999999]">
        <div className='hover:scale-[1.06] transition-all duration-300 relative mx-[0.5rem] px-[0.3rem]'>
        <img src={profile_path !== null ? `https://www.themoviedb.org/t/p/w185/${profile_path}` : "https://templateresources.usu.edu/_assets/images/directory/USU_Placeholder_Directory_AM_3.png" } alt="actor" className="w-full hover:shadow-2xl hover:shadow-[#38383854]" loading="lazy" title={name}/>

        <p className="truncate mt-[0.5rem]">{name || original_name}</p>
        <p className="truncate text-light text-sm">{character}</p>

        <div className='flex justify-between mt-1'>
            <p className='text-light text-sm'>{job || known_for_department}</p>
            <p className="text-white text-sm flex gap-[0.2rem] items-center">
